...
def logout(http_request: Request, response: Response, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for cookie_name in ("access_token", "refresh_token"):
        ...
        if not raw:
            continue
        try:
            # SonarQube CRITICAL: previously used jose_jwt.get_unverified_claims(),
            # which trusts the jti/exp claims without checking the signature —
            # anyone could hand-craft a JWT-shaped cookie to make this endpoint
            # revoke an arbitrary jti of their choosing. decode_token() verifies
            # the signature (and expiry) first; a None result just means there's
            # nothing valid to revoke, which is safe to skip.
